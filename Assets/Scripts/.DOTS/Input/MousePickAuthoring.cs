...
namespace Project.Physics
{
    ...
    public partial class MousePickSystem : SystemBase
    {
        ...
        struct Pick : IJob
        {
            ...
            public void Execute()
            {
                ...
                if (CollisionWorld.CastRay(RayInput, ref mousePickCollector))
                {
					...
					Debug.Log("RaycastHit: " + mousePickCollector.Hit);
                }
                else
                {
                    SpringDataRef.Value = new SpringData
        ...
        protected override void OnUpdate()
        {
			...
			{
				// UI에서 입력을 받고 있을 때 물리 이벤트를 처리하지 않음
				return;
			}
            if (Input.GetMouseButtonDown(0) && (Camera.main != null))
            {
				...
				Debug.Log("entity: " + SpringDataRef.Value.Entity);

				// 의존성 관리: 
				// Dependency는 해당 시스템의 잡이 다른 시스템의 잡과 의존 관계가 있을 때, 이 의존성을 관리합니다. 시스템이 실행되기 전에 다른 잡이 끝나기를 기다려야 할 경우, Dependency를 통해 이를 조정할 수 있습니다.
